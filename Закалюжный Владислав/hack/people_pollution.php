...
{   
    ...
    "<div class='sub-amount sub m'>колличество выбросов (кг в день)Среднее количество</div>".
    "<div class='sub-amount sub m'>колличество выбросов (кг в месяц)Среднее количество</div>".
    "<div class='sub-amount sub m'>колличество выбросов (кг в год)</div>".
    "</div>";
    echo "<table>";
    ...
    for($district=0;$district<count($_DISTRICTS);$district++)
    {
        ...
        $mount_m=$mount_m+round((((int) $amounts[0])+((int)$amounts[1])+((int)$amounts[2])+((int)$amounts[3])+((int)$amounts[4])+((int)$amounts[5])+((int)$amounts[6])+((int)$amounts[7])+((int)$amounts[8])+((int)$amounts[9])+((int)$amounts[10])+((int)$amounts[11]))*$mass_formuls_pollution[$tranport]*((int)$mass_tranport[$district]), 2);
        ...
        if($district==13){
            echo "<div style='background-color:white;border-top:3px solid black; display:flex; justify-content:space-between;'>"
              ...
              "Казахстан"."</div>".
                "<div class='coll'>".ROUND((($mount_m/360)/$max_people),2)."</div>".
                "<div class='coll'>".ROUND(((($mount_m)/12)/$max_people),2)."</div>".
                "<div class='coll'>".ROUND((($mount_m)/$max_people),2)."</div>".
            "</div>";
            }
